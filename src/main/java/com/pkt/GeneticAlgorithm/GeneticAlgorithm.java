...
public class GeneticAlgorithm {
		...
		void calRate(SpeciesPopulation list)
		{
			...
			{
//				System.out.println("pointgeness:" + point.genes[0]+point.genes[1]+point.genes[2]+point.genes[3]);
		...
		{			
			...
			while(point!=null)
			{
				if(talentOrd < point.fitness)
				{
					talentOrd = point.fitness;
					talentSpecies = point;
				}
//				System.out.println("point.fitness:" + point.fitness);
				point=point.next;
			}
		...
		void crossover(SpeciesPopulation list)
		{
			...
			{			
				...
				if(point.next != null)
				{
					int begin=rand.nextInt(TSPData.SUITE_NUM);

					//取point和point.next进行交叉，形成新的两个染色体
					for(int i=begin;i<TSPData.SUITE_NUM;i++)
					{
		...
		{	
			...
			while(point != null)
			{
				...
				if(rate < TSPData.pm)
				{
					//寻找逆转左右端点
					Random rand=new Random();
					int left=rand.nextInt(TSPData.SUITE_NUM);
					int right=rand.nextInt(TSPData.SUITE_NUM);
					if(left > right)
					{
						int tmp;
						tmp=left;
